...
_All three packages (ajv, vitest, prettier) are among the most widely-used packages in the JavaScript ecosystem with no suspicious postinstall scripts. Confidence is HIGH despite slopcheck unavailability._
...
const schema = JSON.parse(
  readFileSync(
    new URL("./schemas/marketplace.schema.json", import.meta.url),
    "utf-8"
  )
);
...
describe("marketplace.json", () => {
  ...
  beforeAll(() => {
    ...
    schema = JSON.parse(
      readFileSync(
        join(__dirname, "schemas", "marketplace.schema.json"),
        "utf-8"
      )
    );
  ...
  it("has unique plugin names", () => {
    ...
    expect(
      duplicates,
      `Duplicate plugin names: ${duplicates.join(", ")}`
    ).toEqual([]);
  });
